...
class RebelsList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(create: (_) => RebelsListState()..init(), child: RebelsListView());
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final rebel = state.value[index];
        return ListTile(
          title: Text(rebel.name, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
          subtitle: Column(
            ...
            children: [
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Standardangriff', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                  Text('Wände durchdringen: ${rebel.defaultAttack.canPenetrateWalls}'),
                  Text('Maximale Wurfweite: ${rebel.defaultAttack.maxThrowingDistance}'),
                  Text('Schadensbereich:'),
                  DamageAreaDisplay(damageArea: rebel.defaultAttack.damageArea),
                ],
              ),
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Multi', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                  Text('Wände durchdringen: ${rebel.multiAttack.canPenetrateWalls}'),
                  Text('Maximale Wurfweite: ${rebel.multiAttack.maxThrowingDistance}'),
                  Text('Bewegung: ${rebel.multiAttack.minMoveDistance} - ${rebel.multiAttack.maxMoveDistance}'),
                  Text('Schadensbereich:'),
        ...
        itemWidgets.add(
          SizedBox(
            ...
            child: Padding(
              ...
              child: Container(color: isInDamageArea ? ((x == 0 && y == 0) ? Colors.red : Colors.red.withAlpha(128)) : Colors.transparent),
            ),
          ),
        );
      }
